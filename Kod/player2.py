...
#funkcija za prijem MQTT poruka
def subFun(topic, msg):
    global randomNumbers, inputString, players, currentPlayer, guessedNumbers, level
    print("Received message on topic:", topic, "with message:", msg) 
    if topic == b'aksk1/randNumP1': #prijem poruke o rand generisanom broju
        randomNumbers = ujson.loads(msg)
        print("Rand num porukica", randomNumbers) 
        gameLoop()
    if topic == b'aksk1/inputStr1': #za prikaz brojeva koje unosi player1
        key = ujson.loads(msg)
        print("Input key od P1:", inputString) 
        showOnDisplay(key)
    if topic == b'aksk1/selectedColor1': #boja avatara playera1
        key=ujson.loads(msg)
        players[0]['color'] = colors[int(key)-1]
        avatarSelection()
        while True:
            key = readKeypad()
            if key in ['1', '2', '3', '4', '5', '6', '7']:
                avatarSelected(key)
                msg=ujson.dumps(key)
                mqtt_conn.publish(b'aksk2/selectedColor2',msg)
                break
    if topic == b'aksk1/pAgain':#ponovno pokretanje igre
        key=ujson.loads(msg)
        level = 1
        randomNumbers = [0] * 10
        players = [{"name": "Player 1", "score": 0, "color": colour1}, {"name": "Player 2", "score": 0, "color": colour2}]
...
mqtt_conn = MQTTClient(client_id='aksk2', server='broker.hivemq.com',user='',password='',port=1883)
mqtt_conn.set_callback(subFun)
mqtt_conn.connect()
mqtt_conn.subscribe(b"aksk1/#")
...
def avatarSelection(): #odabir boje avatara
    ...
    display.rotation = 2
    display.print(f"{players[1]['name']}, choose the color of")
...
def avatarSelected(avatar): #odabir boje igraca 2
    global player_colors, player_ready, currentPlayer, colors
    drawRectangle(display, 135, 117 + (int(avatar) - 1) * 15, 75, 15, color565(255, 255, 255))
    players[1]['color'] = colors[int(avatar) - 1]
...
def check_mqtt_messages(timer): #provj dolaznih MQTT poruka
    mqtt_conn.check_msg()

mqtt_timer = Timer(-1)
mqtt_timer.init(period=50, mode=Timer.PERIODIC, callback=check_mqtt_messages)

def playAgain(): #ponovno pokretanje igre, player 1 pokrece novu igru!
    display.erase()
    display.set_pos(30,90)
    display.set_font(tt24)
...
while True: #provj da li je unesen key sa tastature
    key = readKeypad()
    if key and currentPlayer == 1:
        msg = ujson.dumps(key)
        mqtt_conn.publish(b'aksk2/inputStr2',msg)
        showOnDisplay(key)
    time.sleep(0.1)
